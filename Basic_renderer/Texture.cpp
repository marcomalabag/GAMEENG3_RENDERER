...
Texture::Texture(const wchar_t* fullPath) : Resource(fullPath)
{
	...
	if (SUCCEEDED(res)) {
		res = DirectX::CreateTexture(GraphicsEngine::get()->getRenderSystem()->m_d3d_device, imageData.GetImages(), 
		...
		GraphicsEngine::get()->getRenderSystem()->m_d3d_device->CreateShaderResourceView(texture, &desc, &ShaderResView);
	}
	else
	{
