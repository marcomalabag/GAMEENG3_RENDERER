...
void DeviceContext::clearRenderTargetColor(SwapChainPtr swap_chain, float red, float green, float blue, float alpha)
{
	...
	m_device_context->ClearDepthStencilView(swap_chain->depthView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1, 0);
...
void DeviceContext::setVertexBufferWithoutTexture(const VertexBufferWithoutTexturePtr vertex_buffer)
{
	UINT stride = vertex_buffer->m_size_vertex;
	UINT offset = 0;
	m_device_context->IASetVertexBuffers(0, 1, &vertex_buffer->m_buffer, &stride, &offset);
...
void DeviceContext::drawTriangleListIndexed(UINT index_count, UINT start_vertex_index, UINT start_index_location)
{
	m_device_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
{
	m_device_context->VSSetShaderResources(0, 1, &texture->ShaderResView);
}

void DeviceContext::setTexture(const PixelShaderPtr pixel_shader, const TexturePtr texture)
{
	m_device_context->PSSetShaderResources(0, 1, &texture->ShaderResView);
}

void DeviceContext::setIndexBuffer(const IndexBufferPtr index_buffer)
{
	m_device_context->IASetIndexBuffer(index_buffer->MyBuffer, DXGI_FORMAT_R32_UINT, 0);
}
